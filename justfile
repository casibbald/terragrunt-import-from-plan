...
test:
    cargo test --lib --bins -- --test-threads=1 --nocapture
...
# CI-friendly version that uses existing fixtures if available
test-all:
    cargo build
...
# Original test-all command for local development
test-all-local:
    cargo build
    cargo run -- clean gcp
    cargo run -- clean aws
