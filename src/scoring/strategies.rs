...
impl IdScoringStrategy for GoogleCloudScoringStrategy {
    fn score_attribute(&self, name: &str, definition: &Value, resource_type: &str) -> f64 {
        ...
        match name {
            ...
            _name if _name.ends_with("_id") => 80.0,
            _name if _name.ends_with("_name") => 75.0,
            "project" => 70.0,
            "location" | "region" | "zone" => 65.0,

            // Lower priority - computed or metadata fields
            _name if definition.get("computed").and_then(|v| v.as_bool()).unwrap_or(false) => {
                if _name.contains("url") || _name.contains("link") { 60.0 } else { 45.0 }
            },
